...
function SkillScoreCard({ skill, router }) {
  ...
  const fetchCard = async () => {
    ...
    setOpen(true); setLd(true);
    try {
      const SUPA_URL = process.env.NEXT_PUBLIC_SUPABASE_URL;
      const SUPA_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
      // We'll just navigate to skill page — scorecard loaded there
      router.push(`/skill?q=${encodeURIComponent(skill.skill_name)}`);
    } finally { setLd(false); }
  };
  ...
  return (
    <div style={{
      ...
      onClick={()=>router.push(`/skill?q=${encodeURIComponent(skill.skill_name)}`)}>
        <Tag color="blue">Skill</Tag>
        <span style={{ fontSize:13, fontWeight:600, color:'var(--text)', flex:1 }}>{skill.skill_name}</span>
        <span style={{
          fontSize:11, fontWeight:600, color:'var(--blue-light)',
          background:'var(--blue-dim)', border:'1px solid var(--blue-glow)',
