...
async function getAIInsight(skillName, context) {
  ...
  return { text: data.insight, source: data.source, label: data.label || data.source };
}
...
export default function SkillPage() {
  ...
  useEffect(()=>{
    if (!q) return;
    setLoading(true); setDetails(null); setInsight(''); setAIDone(false); setInsightSource(''); setInsightLabel(''); setTab('coverage');
  ...
  const fetchInsight = async () => {
    ...
    try {
      ...
      const result = await getAIInsight(q, ctx);
      if (result && result.text) {
        setInsight(result.text);
        setInsightSource(result.source || 'unknown');
        setInsightLabel(result.label || result.source || 'AI');
      } else {
        setInsight('No insight returned. Check API configuration.');
      }
      setAIDone(true);
    } catch(err) {
      console.error('fetchInsight error:', err);
      setInsight('Error fetching insight: ' + (err.message || 'unknown error'));
      setAIDone(true);
    }
