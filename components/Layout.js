  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', flexDirection:'column' }}>
      <nav style={{
        ...
        <div onClick={()=>{
          if (router.pathname === '/') {
            window.dispatchEvent(new Event('sve-reset-chat'));
          } else {
            router.push('/');
          }
        }} style={{
        ...
        <div style={{ display:'flex', gap:2, alignItems:'center' }} className="desktop-nav">
          {NAV.map(n => {
            ...
            return (
              <button key={n.href} onClick={()=>{
                  // If already on this page, fire reset event (e.g. Search clears chat)
                  if (router.pathname === n.href) {
                    window.dispatchEvent(new Event('sve-reset-chat'));
                  } else {
                    router.push(n.href);
                  }
